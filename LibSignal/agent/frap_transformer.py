...
from modified_transformers import TransformerEncoder


@Registry.register_model('frap_transformer')
class FRAP_TRANSFORMERAgent(RLAgent):
    '''
    FRAP_TRANSFORMER consists of FRAP and methods for training agents, communicating with environment, etc.
    but has the main compute as a custom Transformer
    '''
    def __init__(self, world, rank):
        ...
        map_name = self.dic_traffic_env_conf.param['network'] #TODO: look into this
    ...
    def _build_model(self):
        ...
        '''
        model = FRAP_TRANSFORMER(self.dic_agent_conf, self.num_actions, self.phase_pairs, self.comp_mask)
        return model
    ...
    def _batchwise(self, samples):
        '''
        _batchwise
        Reconstruct the samples into batch form(last state, current state, reward, action, timestamp).

        :param samples: original samples record in replay buffer
        :return state_t, state_tp, rewards, actions, timestep: information with batch form
        '''
        # (batch_size,12)
        timestep = [item[0] for item in samples]
        ...
        return state_t, state_tp, rewards, actions, timestep
    ...
    def train(self):
        ...
        target_f = self.model((b_t, timestep), train=False) # (batch_size,num_actions)
    ...
    def load_model(self, e):
        ...
        self.model = FRAP_TRANSFORMER(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs, self.comp_mask)
        self.model.load_state_dict(torch.load(model_name))
        self.target_model = FRAP_TRANSFORMER(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs, self.comp_mask)
...
class FRAP_TRANSFORMER(nn.Module):
    '''
    ...
    def __init__(self, dic_agent_conf, output_shape, phase_pairs, competition_mask):
        super(FRAP_TRANSFORMER, self).__init__()
        ...
        self.dropout_rate = 0.1 # Frequency of dropping the input units in the dropout layers
        self.qnetwork = TransformerEncoder(h, d_k, d_v, d_ff, n, dropout_rate) # todone: make Encoder but for torch
    ...
    def _forward(self, states_tuple):
        '''
        states: ([agents, ob_length], timestep)
        ...
        # TransformerEncoder
        transformer_rep = self.qnetwork(q_network_input)
